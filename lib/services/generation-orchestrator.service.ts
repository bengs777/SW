...
const EXECUTOR_HARD_TIMEOUT_MS = timeoutConfig.executorHardMs
const EXECUTOR_STUCK_OPERATION_MS = timeoutConfig.executorStuckOperationMs
...
async function attemptTargetedRepair(input: {
  ...
  const repairPrompt = [
    ...
    "- COMPONENT_CONTRACT_REPAIR: repair the final merged file state, not an isolated temporary artifact. If app/page.tsx renders HeroSection from sections/hero-section.tsx, preserve that local import path and do not switch validation expectations to @/component-registry/hero unless you also import and use the registry component with required props.",
    "- PATH POLICY: every path must normalize and resolve inside the workspace, and must start with src/, app/, components/, sections/, component-registry/, lib/, prisma/, or an allowlisted root file such as package.json, tsconfig.json, next.config.ts, tailwind.config.ts, postcss.config.js, README.md, or .env.example.",
    "- BLOCKED PATHS: never use .., ~, absolute paths, node_modules, .env files, .git, package-lock.json, pnpm-lock.yaml, or yarn.lock.",
    ...
    JSON.stringify(input.plan.allowedFileScope, null, 2),
    "",
    "COMPONENT_CONTRACT_SOURCE_OF_TRUTH:",
    JSON.stringify(componentRegistryPromptPayload(), null, 2),
    "",
