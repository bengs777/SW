...
const FILE_OUTPUT_SYSTEM_PROMPT = [
  ...
  'Required slice JSON schema: {"files":[{"path":"app/page.tsx","content":"full file content"}],"taskGraph":{"operations":[{"action":"create","path":"app/page.tsx","content":"full file content"}]}}.',
  "The root object must contain files and taskGraph only when generating BUILD artifacts. files must be non-empty. taskGraph.operations must be non-empty. Every operation requires action create, modify, delete, or patch plus a canonical path and complete content for create/modify/patch. Do not output commands, diagnostics, metadata, repairs, or summary for BUILD artifacts.",
