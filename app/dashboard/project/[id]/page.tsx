...
function readGenerationTimeoutMs() {
  ...
  return Number.isFinite(value) ? Math.max(900_000, value) : 900000
}
...
type StreamedGeneratedFilesPayload = {
  source?: string
  artifactStatus?: "draft" | "persisted"
...
export default function EditorPage() {
  ...
  const [workspaceArtifactStatus, setWorkspaceArtifactStatus] = useState<WorkspaceArtifactStatus>("empty")
  ...
  const applyGenerationDraft = useCallback(async (
    jobId: string,
    payload?: StreamedGeneratedFilesPayload | null
  ) => {
    const response = await fetch(`/api/generate/jobs/${jobId}/draft`, {
      cache: "no-store",
    })
    ...
    if (!response.ok || !Array.isArray(data.files)) {
      throw new Error(data.error || `Generation draft failed to load (${response.status})`)
    }

    const draftFiles = normalizeWorkspaceFiles(data.files)
    ...
    setRuntimePreviewUrl(null)
    setActiveFileIndex(0)
    setWorkspaceArtifactStatus("draft")
    setIsDirty(false)
    setPreviewValidation({
      ...
      message: "Draft AI siap di Monaco. Sandbox masih memvalidasi sebelum snapshot resmi.",
    })
    setWorkspaceRestoreNotice("Draft AI sudah masuk editor. Sandbox masih memvalidasi sebelum deploy.")
  ...
  const applyStreamedGeneratedFiles = useCallback((rawPayload: unknown, jobId?: string | null) => {
    ...
    if (data?.source !== "persisted") {
      const draftJobId = jobId || activeGenerationJobIdRef.current
      if (data?.draftAvailable && draftJobId) {
        void applyGenerationDraft(draftJobId, data).catch((error) => {
          const message = error instanceof Error ? error.message : "Gagal memuat draft generation."
          pushErrorLog("project", message)
        })
      }
      console.info(JSON.stringify({
        level: "info",
        msg: data?.draftAvailable ? "streamed_draft_fetch_requested" : "streamed_files_ignored",
        projectId,
        reason: data?.draftAvailable ? "draft_artifact_available_before_sandbox" : "explorer_uses_project_api_as_source_of_truth",
        source: data?.source || null,
        draftArtifactId: data?.draftArtifactId || null,
  ...
  }, [applyGenerationDraft, projectId, pushErrorLog, refreshProjectState])
  ...
  const startGenerationStream = useCallback((jobId: string, reconnectAttempt = 0) => {
    ...
    stream.addEventListener("job.files.updated", (event) => {
      try {
        ...
        applyStreamedGeneratedFiles(payload, jobId)
      } catch (error) {
  ...
  useEffect(() => {
    ...
    const loadProject = async () => {
      ...
      setMessages([])
      setGeneratedFiles([])
      setWorkspaceArtifactStatus("empty")
      setPreviewFiles(null)
      setCurrentVersion(0)
      setActiveFileIndex(0)
      setIsGenerating(false)
  ...
  useEffect(() => {
    ...
    if (workspaceArtifactStatus === "draft") {
      if (workspaceAutosaveTimerRef.current !== null) {
        window.clearTimeout(workspaceAutosaveTimerRef.current)
        workspaceAutosaveTimerRef.current = null
      }
      setWorkspaceRestoreNotice("Perubahan draft tersimpan lokal. Tunggu sandbox verified sebelum snapshot resmi.")
      return
    }
    ...
    try {
      if (workspaceArtifactStatus === "draft") {
        ...
        setMessages((prev) => [
          ...
          {
            id: Math.random().toString(36).substring(7),
            content: "Draft tersimpan lokal di editor. Deploy dan snapshot resmi tetap menunggu sandbox verified.",
            role: "assistant",
            timestamp: new Date(),
          },
        ])
  ...
  }, [activeFileIndex, currentVersion, generatedFiles, latestUserPrompt, persistWorkspaceDraft, pushErrorLog, refreshProjectState, saveFiles, workspaceArtifactStatus])
  ...
  const handleDeployToVercel = useCallback(async () => {
    ...
    if (workspaceArtifactStatus === "draft") {
      appendAssistantMessage("Draft belum verified oleh sandbox. Jalankan validasi sampai lolos sebelum Deploy Vercel.")
      setDeployFlow((current) => ({
        ...current,
        vercelStatus: "failed",
        message: "Draft belum verified oleh sandbox.",
      }))
      return
    }
  ...
  }, [appendAssistantMessage, generatedFiles, isDeploying, projectId, pushErrorLog, workspaceArtifactStatus])
  ...
  const handlePushToGitHub = useCallback(async () => {
    ...
    if (workspaceArtifactStatus === "draft") {
      appendAssistantMessage("Draft belum verified oleh sandbox. GitHub push final ditahan sampai snapshot resmi tersedia.")
      setDeployFlow((current) => ({
        ...current,
        githubStatus: "failed",
        message: "Draft belum verified oleh sandbox.",
  ...
  }, [appendAssistantMessage, generatedFiles, isPushingGitHub, projectId, projectName, pushErrorLog, workspaceArtifactStatus])
  ...
  const handleValidatePreview = useCallback(async () => {
    ...
    try {
      ...
      appendAssistantMessage(
        data.ok
          ? workspaceArtifactStatus === "draft"
            ? "Preview validation passed untuk draft. Deploy tetap menunggu sandbox verified."
  ...
  const handleRollbackVersion = useCallback(async (historyId: string) => {
    ...
    try {
      ...
      setWorkspaceArtifactStatus("persisted")
      setPreviewFiles(null)
      setCurrentVersion((version) => version + 1)
      setIsDirty(false)
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden rounded-[1.5rem] border border-border/70 bg-card shadow-sm">
      <EditorHeader
        ...
        isDraft={workspaceArtifactStatus === "draft"}
