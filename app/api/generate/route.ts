...
const MAX_PROVIDER_PROMPT_LENGTH = 42000
...
function buildRecoveryRetryPrompt(
  ...
  return [
    ...
    "Maximum 5 files. Patch only the smallest related task.",
...
function buildProviderExecutionContext(input: {
  ...
  const contextualPrompt = input.promptWithPreviewContext.trim()
  const plainPrompt = input.promptWithAttachments.trim()
  if (!contextualPrompt || contextualPrompt === plainPrompt) {
    return ""
  }

  const title =
    input.promptLanguage === "id"
      ? "KONTEKS_EKSEKUSI_SWIFT"
      : "SWIFT_EXECUTION_CONTEXT"

  return [
    `${title} (source of truth for this generation):`,
    `- Collaboration mode: ${input.collaborationMode}`,
    `- Intent mode: ${input.promptIntent.mode}`,
    "- Use this context to follow the user's requested flow, active file, current files, project memory, and preview errors.",
    "- If any inferred planning text conflicts with this context or the latest user prompt, follow the latest user prompt and this context.",
    "",
    contextualPrompt,
  ].join("\n")
}
...
function buildFinalPrompt({
  ...
  const generator = [
    ...
    "- ONLY implement one feature/module per iteration.",
    "- ONLY modify relevant files.",
    "- Maximum 5 files per response.",
    "- ALWAYS minimize token usage and changed files.",
    ...
    "- Use Next.js Route Handlers and Prisma only when the current task requires backend/persistence.",
    "- Prefer PostgreSQL-compatible Prisma models when adding persistence.",
    "- Keep responsive modern SaaS UI and clean architecture.",
    "",
    "TASK EXECUTION:",
    "- If this is a brand-new project, implement only the first visible useful slice, usually app/layout.tsx and app/page.tsx plus at most tiny supporting files.",
    "- If project files already exist, patch existing files before adding new files.",
    "- Do not add speculative future features; mention next steps in message only.",
  ...
  const governance = [
    ...
    `- generationStage: ${generationStage}`,
    `- promptProfile: ${promptProfile}`,
    "- Planning is lightweight: keep a roadmap internally and execute only the next isolated task.",
    "- Step 1 must preserve a renderable minimal slice: app/layout.tsx, app/page.tsx, and at most small supporting files.",
    "- Step 2 may expand one feature only after the current slice remains valid.",
    "- Do not dump full folder structures or broad architecture.",
    promptProfile === "lightweight"
      ? "- Lightweight profile: keep output tiny, preview-safe, and deterministic. Prefer one visible slice over enterprise architecture."
      : promptProfile === "advanced"
        ? "- Advanced profile: architecture/debug/hardening is allowed, but still implement one task and keep every returned file runnable."
        : "- Balanced profile: one moderate feature slice is allowed; avoid oversized file graphs.",
  ].join("\n")
...
function adaptPromptForGenerationProfile(input: {
  ...
  if (input.profile === "lightweight") {
    return [
      ...
      "Required output: app/layout.tsx, app/page.tsx, and at most small reusable components only.",
      "Maximum 5 files. One feature/module only.",
  ...
  if (input.stage === "scaffold") {
    return [
      ...
      "Maximum 5 files. Do not generate the whole application.",
      "",
      input.prompt,
    ].join("\n")
  }
...
function hardenGeneratedReactAppFiles(files: GeneratedFile[]): GeneratedFile[] {
  const normalized = files.map((file) => {
    ...
    const hasGeneratedBoundary = isGeneratedErrorBoundaryFile(file.path) || definesGeneratedErrorBoundary(originalContent)
    const withoutInvalidMutations = removeInvalidReactObjectMutations(originalContent)
    const withoutStaleBoundaryReferences = hasGeneratedBoundary
      ? withoutInvalidMutations
      : rewriteGeneratedBoundaryReferences(withoutInvalidMutations)
    const safeContent = hasGeneratedBoundary
      ? buildSwiftSafeErrorBoundaryModuleWithAliases()
      : hasInvalidReactObjectMutation(withoutStaleBoundaryReferences)
        ...
        : withoutStaleBoundaryReferences
  ...
  if (!normalized.some((file) => isGeneratedReactPage(file.path))) {
    return normalized
  }

  const withBoundary = normalized.some((file) => normalizeGeneratedPath(file.path) === SWIFT_SAFE_ERROR_BOUNDARY_PATH)
    ? normalized
    : [
        ...normalized,
        {
          path: SWIFT_SAFE_ERROR_BOUNDARY_PATH,
          language: "tsx" as const,
          content: buildSwiftSafeErrorBoundaryModuleWithAliases(),
        },
      ]

  return withBoundary.map((file) => {
    if (!isGeneratedReactPage(file.path)) {
      return file
    }

    return {
      ...file,
      content: wrapGeneratedPageWithSafeBoundary(String(file.content || "")),
    }
  })
}
...
function isGeneratedReactFile(path: string) {
  const normalized = normalizeGeneratedPath(path)
  return GENERATED_REACT_FILE_PATTERN.test(normalized) && !normalized.includes("/api/")
}

function isGeneratedReactPage(path: string) {
...
function definesGeneratedErrorBoundary(content: string) {
...
function hasInvalidReactObjectMutation(content: string) {
  const source = String(content || "")
  return [
    /\b[A-Za-z_$][\w$]*(?:\.[A-Za-z_$][\w$]*)*\.props\s*=/,
    /\b[A-Za-z_$][\w$]*(?:\.[A-Za-z_$][\w$]*)*\.children\.props\s*=/,
    /\b[A-Za-z_$][\w$]*(?:\.[A-Za-z_$][\w$]*)*\.prototype\.props\s*=/,
    /\bObject\.assign\s*\(\s*[^)]*(?:\.props|children\.props|prototype\.props)/,
  ].some((pattern) => pattern.test(source))
}

function removeInvalidReactObjectMutations(content: string) {
  let output = String(content || "")

  const dangerousLinePatterns = [
    /^\s*[A-Za-z_$][\w$]*(?:\.[A-Za-z_$][\w$]*)*\.props\s*=\s*[^;\n]+;?\s*$/gm,
    /^\s*[A-Za-z_$][\w$]*(?:\.[A-Za-z_$][\w$]*)*\.children\.props\s*=\s*[^;\n]+;?\s*$/gm,
    /^\s*[A-Za-z_$][\w$]*(?:\.[A-Za-z_$][\w$]*)*\.prototype\.props\s*=\s*[^;\n]+;?\s*$/gm,
    /^\s*Object\.assign\(\s*[^)]*(?:\.props|children\.props|prototype\.props)[^)]*\)\s*;?\s*$/gm,
  ]
...
function rewriteGeneratedBoundaryReferences(content: string) {
  let output = String(content || "")
  const hadBoundaryReference = /\bErrorBoundary\b|error-boundary/i.test(output)

  if (!hadBoundaryReference) {
    return output
  }

  output = output.replace(/^\s*import\s+[\s\S]*?\bErrorBoundary\b[\s\S]*?from\s+["'][^"']+["'];?\s*$/gm, "")
  output = output.replace(/^\s*import\s+[\s\S]*?from\s+["'][^"']*error-boundary[^"']*["'];?\s*$/gim, "")
  output = output.replace(/^\s*export\s+\{[\s\S]*?\bErrorBoundary\b[\s\S]*?\}\s+from\s+["'][^"']+["'];?\s*$/gm, "")
  output = output.replace(/^\s*export\s+\*?\s*from\s+["'][^"']*error-boundary[^"']*["'];?\s*$/gim, "")
...
function wrapGeneratedPageWithSafeBoundary(content: string) {
  let source = String(content || "")

  if (source.includes("SwiftSafeErrorBoundary")) {
    return source
  }
  ...
  if (namedDefaultFunction?.[2]) {
    const componentName = namedDefaultFunction[2]
    source = source.replace(/export\s+default\s+(async\s+)?function\s+([A-Z][A-Za-z0-9_]*)\s*\(/, (_match, asyncKeyword = "", name) => `${asyncKeyword || ""}function ${name}(`)
    return appendSafePageWrapper(source, componentName)
  }

  const anonymousDefaultFunction = /export\s+default\s+(async\s+)?function\s*\(/.test(source)
  if (anonymousDefaultFunction) {
    source = source.replace(/export\s+default\s+(async\s+)?function\s*\(/, (_match, asyncKeyword = "") => `${asyncKeyword || ""}function SwiftGeneratedPage(`)
    return appendSafePageWrapper(source, "SwiftGeneratedPage")
  }

  const defaultIdentifier = source.match(/export\s+default\s+([A-Z][A-Za-z0-9_]*)\s*;?/)
  if (defaultIdentifier?.[1]) {
    const componentName = defaultIdentifier[1]
    source = source.replace(/export\s+default\s+[A-Z][A-Za-z0-9_]*\s*;?/, "")
    return appendSafePageWrapper(source, componentName)
  }
...
function insertImportAfterDirectives(source: string, importLine: string) {
  ...
  while (insertAt < lines.length && /^\s*(['"])use (client|server)\1\s*;?\s*$/.test(lines[insertAt])) {
    insertAt += 1
  }
...
function appendSafePageWrapper(source: string, componentName: string) {
  return `${source.trimEnd()}\n\nexport default function SwiftSafeGeneratedPage(props: Record<string, unknown>) {\n  return (\n    <SwiftSafeErrorBoundary>\n      <${componentName} {...props} />\n    </SwiftSafeErrorBoundary>\n  )\n}\n`
}

function buildUnsafeReactFallbackModule(filePath: string, detail: string, originalContent = "") {
  const namedExports = extractGeneratedComponentExportNames(originalContent)
    .filter((name) => name !== "SwiftUnsafeReactFallback")
    .map((name) => `export const ${name} = SwiftUnsafeReactFallback`)
    .join("\n")

  return `"use client"\n\nfunction SwiftUnsafeReactFallback() {\n  return (\n    <div className="flex min-h-[240px] items-center justify-center rounded-xl border border-dashed border-border bg-background p-6 text-center">\n      <div className="max-w-xl space-y-2">\n        <h2 className="text-base font-semibold text-foreground">Component fallback: ${JSON.stringify(filePath)}</h2>\n        <p className="text-sm text-muted-foreground">${JSON.stringify(detail)}</p>\n      </div>\n    </div>\n  )\n}\n\n${namedExports ? `${namedExports}\n\n` : ""}export default SwiftUnsafeReactFallback\n`
}

function extractGeneratedComponentExportNames(content: string) {
  const source = String(content || "")
  ...
  const namedExportBlocks = source.matchAll(/export\s*\{([^}]+)\}/g)
  for (const block of namedExportBlocks) {
    for (const item of block[1].split(",")) {
      const alias = item.trim().match(/\bas\s+([A-Z][A-Za-z0-9_]*)$/)
      const direct = item.trim().match(/^([A-Z][A-Za-z0-9_]*)$/)
...
function enforceFullStackRequirement(prompt: string) {
  return [
    prompt,
    "Core role: You are the core orchestration engine for Swift AI, a production-focused incremental full-stack web builder.",
    "Do not generate the entire application in one response. Analyze intent, keep a lightweight roadmap internally, then implement only the next small feature/module.",
    "Auto-detect product intent and apply matching defaults, but scope the response to one useful slice: laundry = first order intake slice; dashboard = first overview slice; ecommerce = first storefront/product slice; landing page = first hero/content slice; booking = first reservation slice; CRM = first records slice.",
    ...
    "For trading, forex, crypto, saham, mata uang, exchange, or currency prompts: implement one financial trading slice such as watchlist + price cards + chart area first. Do not replace it with a generic content page, marketplace, or app-builder workspace.",
    ...
    "Hard requirement: Incremental compile-safe output, not full repository generation.",
    "Include backend API route(s), Prisma schema, or service layer only when the current task requires persistence/backend behavior.",
    "Default stack: Next.js + TypeScript + Tailwind + shadcn/ui (unless user explicitly asks otherwise).",
    "When creating/editing/deleting files, keep imports coherent across related files. If you add a package import, also update package.json dependencies.",
    "If the current task needs Prisma, update prisma/schema.prisma and the directly related route/service files only.",
    "For SQLite/Turso writes, avoid broad updateMany patterns. Prefer targeted writes, retries for SQLITE_BUSY, and short transactions.",
    "Separate generation work from repair work: implement the current task first, then include only targeted repair files if build/runtime errors are known.",
    "For UI tasks, include responsive design, clean navigation/CTA where relevant, loading/empty states only when the current slice needs them, and usable mobile layout.",
    ...
    "Maximum 5 files per response. Return changed files only.",
    ...
    "If building a marketplace such as BelanjaKu/Shopee, start with buyer storefront + product listing or cart mock only, then list Seller Center, Admin CMS, search, notifications, moderation, and payment webhook as next modules.",
...
function compactProviderPrompt(prompt: string) {
  const normalized = String(prompt || "").trim()
  if (normalized.length <= MAX_PROVIDER_PROMPT_LENGTH) {
    return normalized
  }

  return [
    compactText(normalized, MAX_PROVIDER_PROMPT_LENGTH),
    "",
    "FINAL_RESPONSE_CONTRACT:",
    'Return ONLY a valid JSON object with shape {"message":"short summary","files":[{"path":"...","language":"tsx|ts|css|json|html|prisma|md|env","content":"complete file content"}]}.',
    "Include only files that are created or modified, with complete file content.",
    "Maximum 5 files. Execute one isolated task only.",
    "Never regenerate the whole repository.",
    "Preserve the latest user request, active editor context, and preview error context from the retained prompt sections.",
  ].join("\n")
}
...
function buildContinuationPrompt({
  ...
  return [
    ...
    "Return only the files you actually changed.",
    "Maximum 5 files. Implement one feature/module/task only.",
    "Never rewrite unrelated files or regenerate the whole repository.",
